...
/* ============================
   Progress / audio elements
   ============================ */
const audio = document.getElementById("audio"); // <= newly added audio element in HTML
...
// Update progress UI during playback
audio.addEventListener("timeupdate", () => {
    ...
    // persist small save of current time
    userData.songCurrentTime = audio.currentTime;
});

// When song metadata loads, set total duration
audio.addEventListener("loadedmetadata", () => {
    totalTimeEl.textContent = formatTime(audio.duration);
});

// ended behavior already in your code; keep it
...
// calculate seek time from a pointer event
...
progressWrap.addEventListener("mousedown", (e) => {
    ...
    const seekTime = calculateSeekTime(e.clientX);
    progressFilled.style.width = `${(seekTime / audio.duration) * 100}%`;
    progressKnob.style.left = `${(seekTime / audio.duration) * 100}%`;
});
